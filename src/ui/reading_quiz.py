from PyQt5.QtWidgets import QWidget, QLabel, QPushButton, QVBoxLayout, QGridLayout, QMessageBox
from PyQt5.QtCore import Qt, QTimer
from PyQt5.QtGui import QFont
import random

class ReadingQuiz(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle('Kanji Reading Quiz (Onyomi/Kunyomi)')
    ...
    def load_question(self):
        ...
        for btn in self.option_buttons:
            btn.setEnabled(True)
            btn.setStyleSheet("border: 2px solid #444; border-radius: 8px; text-align: center; padding: 8px 12px; background: none; color: white;")
        ...
        onyomi, kunyomi = self.kanji_data[self.current_kanji]
        correct_reading = f"{onyomi} | {kunyomi}"

        other_readings = []
        for k, v in self.kanji_data.items():
            if k != self.current_kanji:
                oy = v[0] if isinstance(v[0], str) else "、".join(v[0]) if isinstance(v[0], list) else str(v[0])
                ku = v[1] if isinstance(v[1], str) else "、".join(v[1]) if isinstance(v[1], list) else str(v[1])
                other_readings.append(f"{oy} | {ku}")

        if len(other_readings) < 3:
            QMessageBox.warning(self, 'Error', 'Not enough kanji data!')
            return

        wrong_readings = random.sample(other_readings, 3)
    ...
    def option_clicked(self, index):
        ...
        onyomi, kunyomi = self.kanji_data[self.current_kanji]
        oy = onyomi if isinstance(onyomi, str) else "、".join(onyomi)
        ku = kunyomi if isinstance(kunyomi, str) else "、".join(kunyomi)
        correct_reading = f"{oy} | {ku}"

        if selected_button.text() == correct_reading:
            ...
            self.current_question += 1
            self.update_score()
            QTimer.singleShot(1000, lambda: self.show_correct_answer(correct_reading))

    def show_correct_answer(self, correct_reading):
        for btn in self.option_buttons:
            if btn.text() == correct_reading:
